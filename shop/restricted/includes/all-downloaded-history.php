...
$filecode = "";
 ...
 if(isset($_GET['code']))
 {
	 ...
	 $query1 =  "SELECT  `id`, `filecode`, `created_date`, `created_by`, `type`, `filename` FROM `downloaded_who` WHERE `filecode`  = '$filecode'   ORDER BY `created_by` DESC";
 }
else
{
	
	$query1 =  "SELECT  `id`, `filecode`, `created_date`, `created_by`, `type`, `filename` FROM `downloaded_who`  ORDER BY `created_by` DESC";
}
