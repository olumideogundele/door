...
$logo = $target_dir.$name.basename($_FILES["image"]["name"]);
  ...
  $extract_user = mysqli_query($conn, "SELECT * FROM `courier_category` WHERE `name` = '$name'") or die(mysqli_error($conn));
...
$sql = 	 "UPDATE `courier_category` SET `name` = '$name', `desc` = '$desc'  , `status` = '1', `created_date` = '$datetime', `registeredby` = '$emailing' , `image` = '$logo'   WHERE `name` = '$name'";
...
$sql = 	 "INSERT INTO `courier_category`(`name`, `desc`, `status`, `created_date`, `registeredby`, `image`) VALUES
