...
if(isset($_GET['value']))
{
	$reff = $_GET['value'];
	 ...
	 $query =  "SELECT `id`, `department`, `product`, `year`, `monthly`, `desc_text`, `status`, `created_date`, `registeredby`, `qty`, `month` FROM `department_income_target_product_monthly` WHERE `reff` = '$reff'";
 	 ...
 	 while ($row_distance1=mysqli_fetch_row($extract_distance1))
    {
	  ...
	  if($privilege == "delete")
		 ...
		 }if($privilege == "edit")
		 {
			 ...
			 $value .= '<li> <a class="dropdown-item"  onClick="return confirm(\'Are you sure you want to update?\')" href=edit-departmental-target-product?id='.$id.'"> Deactivate</a>  </li>';
		 }
...
echo '<tr>
<td><a href = "edit-departmental-target-product?id='.$id.'"><strong>'.$department_name.'</strong></a></td>
<td><a href = "edit-departmental-target-product?id='.$id.'"><strong>'.$product_name.'</strong></a></td>
<td>'.$month.'</td>
<td>'.$year.'</td>
