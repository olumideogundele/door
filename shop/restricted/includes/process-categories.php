...
$name=  mysqli_real_escape_string($conn,$_POST['name']);
  ...
  $extract_user = mysqli_query($conn, "SELECT * FROM `file_category` WHERE  `name` = '$name'") or die(mysqli_error($conn));
		$count = mysqli_num_rows($extract_user);
		 if ($count > 0) {
	 	 //ip` VARCHAR(30) NULL AFTER `registeredby`, ADD `port` 
		 
$sql = 	 "UPDATE `file_category` SET  `name` = '$name',  `status` = '$status', `created_date` = '$datetime', `registeredby` = '$emailing'  WHERE `name` = '$name'";
...
$sql = 	 "INSERT INTO `file_category`(`name` , `status`, `created_date`, `registeredby`) VALUES
