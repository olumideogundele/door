...
$max_amount = $myName->showName($conn, "SELECT  `amount` FROM `max_amount` WHERE `status` = 1"); 	
...
$amount = mysqli_real_escape_string($conn, $_POST['amount']);
	...
	$tag = "No files selected."; 
		 echo '<div class="btn btn-danger btn-lg">Amount requested is greater than petty cash amount.</a></div><br />';	
...
<strong> '.$message.'</strong></span>
 ...
 $query112 = "INSERT INTO `userfiles`(`filepath`, `filename`,`registry`, `name`,`file_type`, `created_date`, `creeated_by`, `status`,`filesize`,`filecode`,`submitted_by`, `group_code`, `signature`, `approval`,`select_stakeholder`, `type`, `account_name`, `account_number`, `bank_name`, `amount`, `petty`) VALUES('uploads','".$target_file2."','".$ministry."','".$name."', 'digital', '$datetime','$emailing', '$status_value', '$size4', '$rand', '$from', '$rand1', '$signature', '$approval', 1, '$type', '$accountname','$acct_num', '$bank', '$amount',1)";
...
if($approval_value == "Yes" and $approval == 1)
{
 ...
 $extract_distance = mysqli_query($conn, $query) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance=mysqli_fetch_row($extract_distance))
    {
		...
		$link = 'https://'.$_SERVER['HTTP_HOST'].'/requipro/petty-approval.php?code='.$rand1;
		 include ("config/DB_config.php"); 
...
<div class="email-background" style="background: #eee;padding: 10px;">
  ...
  <div class="email-container" style="max-width: 500px;background: white;margin: 0 auto;overflow: hidden;border-radius: 5px;text-align: center;font-family: sans-serif;"> <img src="http://'.$_SERVER['HTTP_HOST'].'/requipro/'.$inst_logo.'"  alt="'.$inst_logo.'" width="50px" height="50px" style="max-width: 30%;">
    ...
    <div class="cta" style="padding: 5px;background: #26466D;margin: 0 auto;">
      <p style="padding: 20px;font-weight: 300;font-size: 1.2em;"><a href="https://'.$_SERVER['HTTP_HOST'].'/requipro/petty-approval.php?code='.$rand1.'" style="text-decoration: none;color: white;">Visit Portal Now for More Details</a></p>
