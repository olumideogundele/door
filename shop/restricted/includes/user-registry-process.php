...
$usertype =  mysqli_real_escape_string($conn,$_POST['usertype']);
	 	  $account_number = "DP".rand(10, 99).rand(11, 89).rand(10, 99);		
			 
 
 
  $extract_user = mysqli_query($conn, "SELECT * FROM `user_registry` WHERE  (`name` = '$name' OR `account_number` = '$account_number'  OR `phone` = '$phone' OR `email` = '$email')") or die(mysqli_error($conn));
...
$sql = 	 "INSERT INTO `user_registry`(`usertype`,`name`, `account_number`, `phone`, `email`, `address`, `created_date`, `registeredby`, `status`, `unique_id`, `encrypted_password`, `salt`, `irrelivant`, `ministry`) VALUES
('$usertype','$name', '$account_number', '$phone', '$email', '$address', '$datetime', '$emailing',1, '$uuid' ,'$encrypted_password','$salt', '$password', '$ministry')";
