...
$super = $myName->showName($conn, "SELECT  `super` FROM `user_unit` WHERE `account_number` = '$emailing'"); 
 ...
 $query =  "SELECT  `id`, `truck_type`, `capacity`, `amount`, `status`, `created_date`, `registeredby` FROM `base_fare`  ORDER BY `id` DESC ";	
...
else
{
   ...
   $query =  "SELECT  `id`, `truck_type`, `capacity`, `amount`, `status`, `created_date`, `registeredby` FROM `base_fare`  WHERE `registeredby` = '$emailing' ORDER BY `id` DESC "; 
...
<td>'.$registeredby.'</td>
 ...
 <td> <span class="'.$statusCSS.'">'.$statusParam.' </span> </td>
												...
												<div class="dropdown-menu">
												 ...
												 <a class="dropdown-item" onClick="return confirm(\'Are you sure you want to delete?\')" href="?table=base_fare&del='.$id.'">Delete</a> 
                      
											<div class="dropdown-divider"></div>  <a  class="dropdown-item"   onClick="return confirm(\'Are you sure you want to update?\')" href="?table=base_fare&id='.$id.'&columnValue=1&column=status">Activate</a> 	
											  
											  
											   <a class="dropdown-item"  onClick="return confirm(\'Are you sure you want to update?\')" href="?table=base_fare&id='.$id.'&columnValue=0&column=status"> Deactivate</a>  
