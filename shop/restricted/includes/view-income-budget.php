...
$super = $myName->showName($conn, "SELECT `super` FROM `user_unit` WHERE `account_number` = '$emailing'"); 
			 ...
			 if($super == 1)
			 {
 $query =  "SELECT  `id`,   `year`, `monthly`, `amount`, `desc_text`, `status`, `created_date`, `registeredby`, `reff`,  `super_category`, `category`, `sub_category` FROM `income_budget` ORDER BY `id` DESC";	
				  
			 }
else
{
	$query =  "SELECT `id`,   `year`, `monthly`, `amount`, `desc_text`, `status`, `created_date`, `registeredby`, `reff`,  `super_category`, `category`, `sub_category` FROM `income_budget` WHERE `registeredby` = '$emailing' ORDER BY `id` DESC";	
 	 ...
 	 while ($row_distance=mysqli_fetch_row($extract_distance))
    {
		 ...
		 $year =$row_distance[1];
				  ...
				  $monthly =$row_distance[2];
					    ...
					    $reff =$row_distance[8];
					    $super_category =$row_distance[9];
					    $category =$row_distance[10];
					    $sub_category =$row_distance[11];
 ...
 $extract_distance1 = mysqli_query($conn, $query1) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance1=mysqli_fetch_row($extract_distance1))
    {
		 ...
		 if($privilege == "delete")
		 {
			 ...
			 $value .= '<li> <a class="dropdown-item" onClick="return confirm(\'Are you sure you want to delete?\')" href="?table=income_budget&del='.$id.'">Delete</a> </li>';
		 }
	
		  if($privilege == "activate")
		 {
			 
			 $value .= '<li> <a  class="dropdown-item"   onClick="return confirm(\'Are you sure you want to update?\')" href="?table=income_budget&id='.$id.'&columnValue=1&column=status">Activate</a> </li>';
		 }
  
		  if($privilege == "deactivate")
		 {
			 
			 $value .= '<li> <a class="dropdown-item"  onClick="return confirm(\'Are you sure you want to update?\')" href="?table=income_budget&id='.$id.'&columnValue=0&column=status"> Deactivate</a>  </li>';
		 }
