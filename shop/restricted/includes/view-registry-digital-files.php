...
if($userT == 1)
{
 ...
 $query =  "SELECT  `id`, `filepath`, `filepath`, `ministry`, `agency`, `directoryname`, `shelf`, `shelf_cell`, `name`, `file_type`,`status`,`registry`,`filecode`    FROM `userfiles` WHERE   `file_type` = 'digital' AND (`registry` = '$ministry'   or `creeated_by` = '$emailing' ) ORDER BY `id` DESC";	
	
 
}
else
{
	 $query =  "SELECT  `id`, `filepath`, `filepath`, `ministry`, `agency`, `directoryname`, `shelf`, `shelf_cell`, `name`, `file_type`,`status`,`registry`,`filecode`    FROM `userfiles` WHERE (`file_type` != 'physical') AND (`creeated_by` = '$emailing') AND `status` = 0 ORDER BY `id` DESC";	
...
<div class="checkbox checkbox-primary">
 <input id="checkbox2" type="checkbox"  value = '.$id.'  name = "transfer[]">
 	...
 	</td> <td> <a href="view-file-information.php?file='.$id.'"><strong>'.$name.'</strong></a></td>
 <td> <strong>'.$registry.'</strong> </td>

 <td> <a href="view-file-information.php?file='.$id.'">'.$filecode.' - '.$id.'</a></td>
