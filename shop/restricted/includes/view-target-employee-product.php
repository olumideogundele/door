...
if($super == 1)
{
 ...
 $query =  "SELECT `id`, `department`, `product`, `year`, `monthly`, `desc_text`, `status`, `created_date`, `registeredby`, `reff`, `qty`, `employee` FROM `employee_income_target_product`   ORDER BY `id` DESC";	
				  
}
else
{
	
	$query =  "SELECT `id`, `department`, `product`, `year`, `monthly`, `desc_text`, `status`, `created_date`, `registeredby`, `reff`, `qty`, `employee` FROM `employee_income_target_product` WHERE `registeredby` = '$emailing' ORDER BY `id` DESC";	
...
$department_name = $myName->showName($conn, "SELECT `name` FROM `user_unit` WHERE `account_number` = '$employee'"); 
...
echo '<tr>
<td><a href = "employee-monthly-target-product?value='.$reff.'"><strong>'.$department_name.'</strong></a></td>
<td>'.$year.'</td>
<td>'.$product_name.'</td>
<td>'.$qty.'</td>
