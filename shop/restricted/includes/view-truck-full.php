...
if(isset($_GET['id']))
{
 ...
 {
$query =  "SELECT `id`, `account_number`, `truck_brand`, `year`, `truck_plate_number`, `state`, `total_capacity`, `truck_type`, `calibration_chart`, `road_worthiness_cert`, `commercial_licence`, `git_insurance`, `front_view_1`, `front_view_2`, `side_view_1`, `side_view_2`, `status`, `created_date`, `registeredby`, `driver`, `ccaution`, `extinguisher`, `jacket`, `extratyre`, `hat`, `boot`, `inspector`, `location`, `status_value`  FROM `truck`  WHERE `id` = '$value'	ORDER BY `id` DESC";
}
else
{
$query =  "SELECT `id`, `account_number`, `truck_brand`, `year`, `truck_plate_number`, `state`, `total_capacity`, `truck_type`, `calibration_chart`, `road_worthiness_cert`, `commercial_licence`, `git_insurance`, `front_view_1`, `front_view_2`, `side_view_1`, `side_view_2`, `status`, `created_date`, `registeredby`, `driver`, `ccaution`, `extinguisher`, `jacket`, `extratyre`, `hat`, `boot`, `inspector`, `location`, `status_value`  FROM `truck` WHERE `registeredby` = '$emailing' AND `id` = '$value' ORDER BY `id` DESC";
 }
  $extract_distance = mysqli_query($conn, $query) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance=mysqli_fetch_row($extract_distance))
    {
         ...
         $ccaution =$row_distance[20];
					   		...
					   		$location =$row_distance[27];
					   		$status_value =$row_distance[28];
         ...
         $state_name = $myName->showName($conn, "SELECT  `name` FROM `states` WHERE `state_id` = '$state'"); 
          ...
          $truck_type = $myName->showName($conn, "SELECT  `name` FROM  `truck_type` WHERE  `id` = '$truck_type'");
           ...
           $truck_owner_name = $myName->showName($conn, "SELECT  `name` FROM  `user_unit` WHERE  `account_number` = '$account_number'");
