...
$query = "";
 ...
 $query =  "SELECT `id`, `name`, `status`, `created_date`,`registeredby`,`desc_text`,`code`   FROM `types`  ORDER BY `id` DESC";	
 ...
 $extract_distance1 = mysqli_query($conn, $query1) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance1=mysqli_fetch_row($extract_distance1))
    {
		 ...
		 if($privilege == "delete")
		 {
			 ...
			 $value .= '<li> <a class="dropdown-item" onClick="return confirm(\'Are you sure you want to delete?\')" href="?table=types&del='.$id.'">Delete</a> </li>';
		 }
	
		  if($privilege == "activate")
		 {
			 
			 $value .= '<li> <a  class="dropdown-item"   onClick="return confirm(\'Are you sure you want to update?\')" href="?table=types&id='.$id.'&columnValue=1&column=status">Activate</a> </li>';
		 }
  
		  if($privilege == "deactivate")
		 {
			 
			 $value .= '<li> <a class="dropdown-item"  onClick="return confirm(\'Are you sure you want to update?\')" href="?table=types&id='.$id.'&columnValue=0&column=status"> Deactivate</a>  </li>';
		 }
