...
$emailing  = $_SESSION['email'];
	 ...
	 $extract_user = mysqli_query($conn, "SELECT * FROM `userfiles` WHERE  (`stage` != '$position' AND `group_code` = '$code')") or die(mysqli_error($conn));
...
('$code', '$status', '$datetime', '$emailing')";
 ...
 $sqlii = "UPDATE `userfiles` SET  `status` = '$status', `stage` = '$newStage'  WHERE   `group_code` = '$code' ";
 ...
 $query =  "SELECT  `id`, `name`, `phone`, `email` FROM `stake_holder` WHERE `status` = 1 AND `position` = '$newStage'";	
						 ...
						 $sqlii = "UPDATE `userfiles` SET  `status` = '4',`status_a` = '1', `stage` = '$maxposition'  WHERE   `group_code` = '$code' ";
 ...
 $extract_distance = mysqli_query($conn, $query) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance=mysqli_fetch_row($extract_distance))
    {
		...
		$email =$row_distance[3];
		 ...
		 $name = $myName->showName($conn, "SELECT  `name` FROM `userfiles` WHERE `group_code` = '$code'"); 
		
		 
		 $registry = $myName->showName($conn, "SELECT  `registry` FROM `userfiles` WHERE `group_code` = '$code'"); 
...
<div class="email-background" style="background: #eee;padding: 10px;">
  ...
  <div class="email-container" style="max-width: 500px;background: white;margin: 0 auto;overflow: hidden;border-radius: 5px;text-align: center;font-family: sans-serif;"> <img src="http://'.$_SERVER['HTTP_HOST'].'/requipro/'.$inst_logo.'"  alt="'.$inst_logo.'" width="407" height="30%" style="max-width: 30%;">
    ...
    <h1 style="5px;font-size: 2.0em;">Hi     '.$naming.'   </h1>
...
$registeredby = $myName->showName($conn, "SELECT  `registeredby` FROM `userfiles`  WHERE  `group_code` = '$code'"); 	
					 
					 
					 
					 
 $sqlii = "UPDATE `userfiles` SET  `status` = '$status'  WHERE   `group_code` = '$code' ";
